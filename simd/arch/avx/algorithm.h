...
struct min<T, W, REQUIRE_INTEGRAL(T)>
{
    ...
    static Vec<T, W> apply(const Vec<T, W>& lhs, const Vec<T, W>& rhs) noexcept
    {
        ...
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = detail::forward_sse_op<detail::sse_min, sse_vec_t>
...
struct min<float, W>
{
    ...
    static Vec<float, W> apply(const Vec<float, W>& lhs, const Vec<float, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_min_ps(lhs.reg(idx), rhs.reg(idx));
        }
        return ret;
    }
};
...
struct min<double, W>
{
    ...
    static Vec<double, W> apply(const Vec<double, W>& lhs, const Vec<double, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_min_pd(lhs.reg(idx), rhs.reg(idx));
        }
        return ret;
    }
};
...
struct max<float, W>
{
    ...
    static Vec<float, W> apply(const Vec<float, W>& lhs, const Vec<float, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_max_ps(lhs.reg(idx), rhs.reg(idx));
        }
        return ret;
    }
};
...
struct max<double, W>
{
    ...
    static Vec<double, W> apply(const Vec<double, W>& lhs, const Vec<double, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_max_pd(lhs.reg(idx), rhs.reg(idx));
        }
        return ret;
    }
};
...
struct all_of<float, W>
{
    ...
    static bool apply(const VecBool<float, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret = ret && (_mm256_movemask_ps(x.reg(idx)) == 0xFF);
        }
        return ret;
    }
};
...
struct all_of<double, W>
{
    ...
    static bool apply(const VecBool<double, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret = ret && (_mm256_movemask_pd(x.reg(idx)) == 0x0F);
        }
        return ret;
    }
};
...
struct all_of<T, W, REQUIRE_INTEGRAL(T)>
{
    ...
    static bool apply(const VecBool<T, W>& x) noexcept
    {
        ...
        for (auto idx = 0; ret && idx < nregs; idx++) {
            auto result = detail::forward_sse_op0<detail::sse_all_of, bool, sse_vbool_t>
                                (x.reg(idx));
            ret = ret && result.first && result.second;
        }
        return ret;
    }
};
...
struct any_of<float, W>
{
    ...
    static bool apply(const VecBool<float, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret = ret || (_mm256_movemask_ps(x.reg(idx)) != 0);
        }
        return ret;
    }
};
...
struct any_of<double, W>
{
    ...
    static bool apply(const VecBool<double, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret = ret || (_mm256_movemask_pd(x.reg(idx)) != 0);
        }
        return ret;
    }
};
...
struct any_of<T, W, REQUIRE_INTEGRAL(T)>
{
    ...
    static bool apply(const VecBool<T, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret = ret || (!_mm256_testz_si256(x.reg(idx), x.reg(idx)));
        }
        return ret;
    }
};
...
struct select<T, W, REQUIRE_INTEGRAL(T)>
{
    ...
    static Vec<T, W> apply(const VecBool<T, W>& cond, const Vec<T, W>& lhs, const Vec<T, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_or_si256(
                            _mm256_and_si256(cond.reg(idx), lhs.reg(idx)),
                            _mm256_andnot_si256(cond.reg(idx), rhs.reg(idx))
                        );
        }
        return ret;
    }
...
struct select<float, W>
{
    ...
    static Vec<float, W> apply(const VecBool<float, W>& cond, const Vec<float, W>& lhs, const Vec<float, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_blendv_ps(rhs.reg(idx), lhs.reg(idx), cond.reg(idx));
        }
        return ret;
    }
};
...
struct select<double, W>
{
    ...
    static Vec<double, W> apply(const VecBool<double, W>& cond, const Vec<double, W>& lhs, const Vec<double, W>& rhs) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret.reg(idx) = _mm256_blendv_pd(rhs.reg(idx), lhs.reg(idx), cond.reg(idx));
        }
        return ret;
    }
};
...
struct popcount<float, W>
{
    ...
    static int apply(const VecBool<float, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret += bits::count1(_mm256_movemask_ps(x.reg(idx)));
        }
        return ret;
    }
};
...
struct popcount<double, W>
{
    ...
    static int apply(const VecBool<double, W>& x) noexcept
    {
        ...
        #pragma unroll
        for (auto idx = 0; idx < nregs; idx++) {
            ret += bits::count1(_mm256_movemask_pd(x.reg(idx)));
        }
        return ret;
    }
};
...
namespace detail {
SIMD_INLINE
static int avx_count1_mask_epi16(const avx_reg_i& x) noexcept
{
    /// _mm256_movemask_epi16 not available in avx
    return bits::count1(_mm256_movemask_epi8(x)) >> 1;
}

SIMD_INLINE
static int avx_count1_mask_epi32(const avx_reg_i& x) noexcept
{
    /// _mm256_movemask_epi32 not available in avx
    return bits::count1(_mm256_movemask_ps(_mm256_castsi256_ps(x)));
}

SIMD_INLINE
static int avx_count1_mask_epi64(const avx_reg_i& x) noexcept
{
    /// _mm256_movemask_epi64 not available in avx
    return bits::count1(_mm256_movemask_pd(_mm256_castsi256_pd(x)));
}
}  // namespace detail
...
struct popcount<T, W, REQUIRE_INTEGRAL(T)>
{
    ...
    static int apply(const VecBool<T, W>& x) noexcept
    {
        ...
        SIMD_IF_CONSTEXPR(sizeof(T) == 1) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret += bits::count1(_mm256_movemask_epi8(x.reg(idx)));
            }
        } else SIMD_IF_CONSTEXPR(sizeof(T) == 2) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret += detail::avx_count1_mask_epi16(x.reg(idx));
            }
        } else SIMD_IF_CONSTEXPR(sizeof(T) == 4) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret += detail::avx_count1_mask_epi32(x.reg(idx));
            }
        } else SIMD_IF_CONSTEXPR(sizeof(T) == 8) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret += detail::avx_count1_mask_epi64(x.reg(idx));
            }
        }
        return ret;
    }
