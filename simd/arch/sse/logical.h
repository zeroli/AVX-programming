...
namespace detail {
inline static sse_reg_i bitwise_slli_epi8(const sse_reg_i& x, int32_t y)
{
    return _mm_and_si128(_mm_set1_epi8(0xFF << y), _mm_slli_epi32(x, y));
}
}  // namespace detail
...
struct bitwise_lshift<T, W, REQUIRE_INTEGRAL(T)>
{
    ...
    static Vec<T, W> apply(const Vec<T, W>& x, int32_t y) noexcept
    {
        ...
        SIMD_IF_CONSTEXPR(sizeof(T) == 1) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret.reg(idx) = detail::bitwise_slli_epi8(x.reg(idx), y);
            }
...
namespace detail {
SIMD_INLINE
static sse_reg_i bitwise_sra_epi8(const sse_reg_i& x, int32_t y)
{
    sse_reg_i sign_mask = _mm_set1_epi16((0xFF00 >> y) & 0x00FF);
    sse_reg_i cmp_is_negative = _mm_cmpgt_epi8(_mm_setzero_si128(), x);
    sse_reg_i res = _mm_sra_epi16(x, _mm_set1_epi64x(y));
    return _mm_or_si128(_mm_and_si128(sign_mask, cmp_is_negative),
                        _mm_andnot_si128(sign_mask, res));
}
SIMD_INLINE
static sse_reg_i bitwise_srl_epi8(const sse_reg_i& x, int32_t y)
{
    return _mm_and_si128(_mm_set1_epi8(0xFF >> y),
                         _mm_srl_epi32(x, _mm_set1_epi64x(y)));
}
SIMD_INLINE
static sse_reg_i bitwise_sra_epi64(const sse_reg_i& x, int32_t y)
{
    return _mm_or_si128(
            _mm_srl_epi64(x, _mm_set1_epi64x(y)),
            _mm_sll_epi64(
                _mm_srai_epi32(_mm_shuffle_epi32(x, _MM_SHUFFLE(3, 3, 1, 1)), 32),
                _mm_set1_epi64x(64 - y)
            )
        );
}
SIMD_INLINE
static sse_reg_i bitwise_srl_epi64(const sse_reg_i& x, int32_t y)
{
    return _mm_srl_epi64(x, _mm_set1_epi64x(y));
}
}  // namespace detail
        ...
        SIMD_IF_CONSTEXPR(sizeof(T) == 1) {
            ...
            for (auto idx = 0; idx < nregs; idx++) {
                ret.reg(idx) = is_signed
                    ? detail::bitwise_sra_epi8(x.reg(idx), y)
                    : detail::bitwise_srl_epi8(x.reg(idx), y);
            }
        } else SIMD_IF_CONSTEXPR(sizeof(T) == 2) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret.reg(idx) = is_signed
                    ? _mm_sra_epi16(x.reg(idx), _mm_set1_epi64x(y))
                    : _mm_srl_epi16(x.reg(idx), _mm_set1_epi64x(y));
            }
        } else SIMD_IF_CONSTEXPR(sizeof(T) == 4) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret.reg(idx) = is_signed
                    ? _mm_sra_epi32(x.reg(idx), _mm_set1_epi64x(y))
                    : _mm_srl_epi32(x.reg(idx), _mm_set1_epi64x(y));
            }
        } else SIMD_IF_CONSTEXPR(sizeof(T) == 8) {
            #pragma unroll
            for (auto idx = 0; idx < nregs; idx++) {
                ret.reg(idx) = is_signed
                    ? detail::bitwise_sra_epi64(x.reg(idx), y)
                    : detail::bitwise_srl_epi64(x.reg(idx), y);
            }
        }
        return ret;
    }
